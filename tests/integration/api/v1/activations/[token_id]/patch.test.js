...
describe("PATCH /api/v1/activations/[token_id]", () => {
  describe("Anonymous user", () => {
    test("With nonexistent token", async () => {
      const response = await fetch(
        "http://localhost:3000/api/v1/activations/afbbaeec-138e-43f8-9ab2-186f85a74111",
        {
          method: "PATCH",
        },
      );
    ...
    test("With valid token but already activated user", async () => {
      const createdUser = await orcherstrator.createUser();
      await orcherstrator.activateUser(createdUser);
      const activationToken = await activation.create(createdUser.id);
  ...
  describe("Default user", () => {
    test("With valid token, but already logged in user", async () => {
      const user1 = await orcherstrator.createUser();
      await orcherstrator.activateUser(user1);
      const user1SessionObject = await orcherstrator.createSession(user1.id);

      const user2 = await orcherstrator.createUser();
